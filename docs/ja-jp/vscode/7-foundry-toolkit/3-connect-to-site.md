...
これで、省略可能な機能として、テスト済みのホスト環境のコンシェルジュが、サーバー側プロキシ経由でアクセシビリティに対応したローカルサイトに接続されました。このサイト統合は本番環境へのデプロイではありません。

1. Tailspin Toys リポジトリの `foundry-agent-vscode` で、プロキシ、ウィジェット、テストをレビューして保存します。資格情報やローカル環境ファイルが含まれていないことを確認します。
2. 作業が終わったら、[リソースをクリーンアップする][cleanup]を完了します。該当する場合に限り、記録したデプロイディレクトリと環境を使います。ローカルプロセスの停止だけでなく、Azure での削除を確認します。
3. 3 つのチェックポイントを確認するには[省略可能: Foundry を組み込む][overview]に戻ります。または [VS Code の概要][vscode-overview]に戻ります。必須のコースは演習 6 で完了しています。
