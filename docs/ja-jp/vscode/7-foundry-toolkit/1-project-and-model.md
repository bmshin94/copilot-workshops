...
このチェックポイントで得られるのは、エクスポート済みのカタログと、6 つのテストプロンプトに合格したデプロイ済みモデルです。まだエージェントのデプロイはしていません。

1. Tailspin Toys リポジトリの `foundry-agent-vscode` にカタログの作業を保存します。資格情報とローカル設定は含めません。次のモジュール用に、サブスクリプション、リソースグループ、プロジェクト、リージョン、モデルのデプロイ名を記録します。
2. 同じ VS Code ワークスペースとブランチで、`tailspin-toys` とテスト済みモデルを再利用し、[エージェントを構築してデプロイする][next-lesson]に進みます。ここで終了する場合は、[リソースをクリーンアップする][cleanup]を完了します。エージェントのひな形は必要ありません。
