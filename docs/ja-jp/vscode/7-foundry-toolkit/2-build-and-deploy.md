...
このモジュールの成果は、ローカルでデバッグ済みのエージェントと、カタログの情報範囲を守って稼働するホスト環境へのデプロイです。

1. 生成されたエージェントとテストを `foundry-agent-vscode` に保存します。資格情報とローカル環境ファイルは含めません。統合とクリーンアップのために、ホスト型エージェントの接続情報、生成されたデプロイディレクトリ、`azd` 環境を記録します。シークレットはコミットしません。
2. 同じリポジトリ、ブランチ、VS Code ワークスペースで、[エージェントをサイトに接続する][next-lesson]に進みます。次のモジュールでは、新しいプロジェクトではなく、このデプロイ済みエージェントを使う通常の **Agent** チャットを開始します。ここで終了する場合は、[リソースをクリーンアップする][cleanup]の手順に従います。
