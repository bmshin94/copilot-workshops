...
이 단계의 결과물은 내보낸 카탈로그와 여섯 가지 테스트 프롬프트를 통과한 배포 모델이며, 아직 배포한 에이전트는 아닙니다.

1. 자격 증명과 로컬 설정을 제외하고 카탈로그 작업을 Tailspin Toys 리포지토리의 `foundry-agent-vscode`에 저장합니다. 다음 모듈을 위해 구독, 리소스 그룹, 프로젝트, 지역, 모델 배포 이름을 기록합니다.
2. 동일한 VS Code 작업 영역과 브랜치에서 `tailspin-toys`와 테스트한 모델을 재사용하여 [에이전트 빌드 및 배포][next-lesson]를 계속 진행합니다. 여기서 중단한다면 [리소스 정리][cleanup]를 완료합니다. 에이전트 스캐폴드(Scaffold)는 필요하지 않습니다.
