...
이 단계의 결과물은 로컬에서 디버깅한 에이전트와 카탈로그의 정보 범위를 지키며 실행 중인 호스팅 배포입니다.

1. 자격 증명과 로컬 환경 파일을 제외하고 생성한 에이전트와 테스트를 `foundry-agent-vscode`에 저장합니다. 통합과 정리에 필요한 호스팅 에이전트 연결 정보, 생성된 배포 디렉터리, `azd` 환경을 기록하되 비밀 정보는 커밋하지 않습니다.
2. 동일한 리포지토리, 브랜치, VS Code 작업 영역에서 [사이트에 에이전트 연결][next-lesson]을 계속 진행합니다. 해당 모듈에서는 새 프로젝트가 아니라 이미 배포한 에이전트를 사용하는 일반 **Agent** 채팅을 시작합니다. 여기서 중단한다면 [리소스 정리][cleanup]를 따릅니다.
