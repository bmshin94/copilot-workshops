...
이제 선택 기능은 서버 측 프록시를 통해 테스트한 호스팅 안내 도우미를 접근성 있는 로컬 사이트에 연결합니다. 이는 사이트 통합의 프로덕션 배포가 아닙니다.

1. Tailspin Toys 리포지토리의 `foundry-agent-vscode`에서 프록시, 위젯, 테스트를 검토하고 저장합니다. 자격 증명이나 로컬 환경 파일이 포함되지 않았는지 확인합니다.
2. 작업을 마치면 [리소스 정리][cleanup]를 완료하고, 해당하는 경우에만 기록한 배포 디렉터리와 환경을 사용합니다. 로컬 프로세스 종료뿐만 아니라 Azure에서 실제로 삭제했는지 검증합니다.
3. 세 가지 완료 점검 항목을 보려면 [선택 사항: Foundry 통합][overview]으로 돌아가거나 [VS Code 개요][vscode-overview]로 이동합니다. 필수 과정은 실습 6에서 이미 완료했습니다.
