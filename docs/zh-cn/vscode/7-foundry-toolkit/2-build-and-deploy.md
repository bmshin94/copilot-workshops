...
本模块的成果是经过本地调试的代理，以及正在运行且遵守目录边界的托管部署。

1. 将生成的代理和测试保存到 `foundry-agent-vscode` 分支，不要包含凭据或本地环境文件。记录托管代理的连接详情、生成的部署目录和 `azd` 环境，供集成和清理使用，但不要提交机密。
2. 在同一个存储库、分支和 VS Code 工作区中继续[将代理连接到网站][next-lesson]。该模块会开启普通的 **Agent** 聊天，使用此已部署的代理，而不是新项目。如果在此停止，请执行[清理资源][cleanup]。
