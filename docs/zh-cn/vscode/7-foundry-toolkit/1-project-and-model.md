...
本模块的检查点是已导出的目录，以及通过六个测试提示的已部署模型，此时尚未部署代理。

1. 在 Tailspin Toys 存储库的 `foundry-agent-vscode` 分支上保存目录相关工作，不要包含凭据和本地设置。记录订阅、资源组、项目、区域和模型部署名称，供下一个模块使用。
2. 在同一个 VS Code 工作区和分支中继续[构建并部署代理][next-lesson]，复用 `tailspin-toys` 和已测试的模型。如果在此停止，请完成[清理资源][cleanup]；无需生成代理框架。
