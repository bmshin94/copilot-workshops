...
该可选功能现已通过服务器端代理服务，将经过测试的托管推荐助手连接到无障碍本地网站。这不是网站集成的生产部署。

1. 在 Tailspin Toys 存储库的 `foundry-agent-vscode` 分支上审阅并保存代理服务、小组件和测试。确认未包含凭据或本地环境文件。
2. 完成后执行[清理资源][cleanup]，仅在适用时使用之前记录的部署目录和环境。验证 Azure 资源已删除，而不只是关闭本地进程。
3. 返回[可选：集成 Foundry][overview]查看三个检查点，或返回 [VS Code 概述][vscode-overview]。必修学习路径已在练习 6 后完成。
